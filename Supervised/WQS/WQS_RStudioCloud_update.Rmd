...
b12 = result6rh$coefmat[,16]
b2 = b1+b12
quantile(b1, prob=c(0.025, 0.5, 0.975))
quantile(b2, prob=c(0.025, 0.5, 0.975))
quantile(b12, prob=c(0.025, 0.5, 0.975))

# Plot weights from repeated holdout
to_plot <- result6rh$wmat %>% as_tibble()

pats <- c("_1|_0")

to_plot %>% 
  pivot_longer(cols=LBX074LA_0:LBXPCBLA_1, 
               names_to ="chemical_sex", values_to="weights") %>% 
  mutate(sex = case_when(
    str_detect(chemical_sex, "_1") ~ "Male", 
    str_detect(chemical_sex, "_0") ~ "Female")) %>% 
  mutate(chemical = str_remove(chemical_sex, pats)) %>% 
  arrange(desc(weights)) %>% 
  ggplot(aes(y=weights, x=fct_reorder(chemical, desc(weights)), group=chemical)) +
  geom_point(alpha=0.4,color="blue") +
  geom_boxplot(alpha=0.5) +
  facet_wrap(~sex, ncol= 1, axis.labels = "all") + 
  theme(
