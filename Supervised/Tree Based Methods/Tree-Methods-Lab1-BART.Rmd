...
We can estimate an IQR change in furan 1 when all other variables (exposures and covariates) are fixed at the sample means. The general idea is to create a data frame with two observations. Each observation represents one potential exposure profile. In our case, these exposure profiles will have all variables (exposures and covariates) set to their means, with the exception on furan 1. For furan 1, one observations will have exposure at the 75th percentile and one at the 25th percentile. We will compare the estimated mean response for observations with these two exposure profiles. This can be generalized to any exposure (or covariate) profiles. There is nothin special about setting these to the mean.

The first step is to setup the data. The data should have:
  - two rows, one for each quantile of furan 1 we are considering, 
  - one column for each variable in the training data,
  - all variable set to the mean with the exception of furan 1, and
  - furan 1 set to the quantiles we which to compare (25th in one row and 75th in the other row).
...
Second, we predict with this new data. This is done with the built in predict function from the `bart` package.
...
We now have a data frame with one row for each MCMC iteration (should be 2000) and one column for each of the exposure values we are comparing. We can check this by making a trace plot.
...
Third, we take the difference at each iteration. Each row in the posterior is one MCMC iteration. We can subtract the prediction at the 75th quantile from the prediction at the 25th quantile in each row. This will give us a vector with one number for each MCMC iteration. This is the posterior sample of the difference in response between these exposure scenarios. 
...
Last, we summarize the posterior distribution of the difference. We can summarize this in any way we want.
...
We can estimate an IQR change in furan 1 averaged over all other covaraites. This is similar to the last example, but has a few extra steps. 

Like before, we start by setting up the data. This time we do it separately for each level of furan 1. We also use the full dataset for prediction not just the means. The first step is to create two copies of the dataset, one for each quantile. For each dataset we will set furan 1 to either the 25th or 75th percentiles. All other variables remain the same as in the exposure dataset.
...
Third, we take the average of all 1003 observations for all datasets at each iteration. Each row in the posterior is one iteration and we want to keep these iterations linked throughout. This is done separately for each of our replicate datasets. The result is a vector with one observation for each MCMC iteration. We can interpret this as the posterior sample (MCMC sample) of the mean response in our sample if everyone had furan 1 fixed at the specified quantile and their other variables were as observed in the data. This naturally accounts for effect heterogeneity.
...
Then we take the difference at each iteration, just like before. Each row in the posterior is one iteration.
...
Last, we summarize the posterior distribution of the difference, just like before.
...
BART naturally accounts for heterogeneity. Because we included all exposures and covariates into the model there can be interactions both among exposures and among exposures and covariates. There are two approaches to estimate subgroup specific effects
  1.  Estimate the exposure effects with each subgroup. Using this approach, the effect in each group is conditional of the distribution of covariates and co-exposures observed in each group, which may be different among groups.
  2.  Use a g-computation-type approach to balance covariates and co-exposures across the groups.
  
  
First, we can estimate the exposure-response relationship in different subgroups (approach 1 above). Below we estimate the partial dependence function for Furan 1 among males and females. This is from the same model fit as before. We are simply using the posterior sample to estimate a different estimand.
...
An alternative approach is to use the full dataset but set all individuals to male or female. This eliminates differences in the distribution of covariates between the two groups (e.g. differences in age, BMI, exposures, etc.). This is very similar to the approach used above to average over levels of co-exposures and covariates.
